...
describe('Step function Message', () => {
  ...
  describe('Step function Message', () => {
    it('wrap should behave like Camunda Bpm Engine', () => {
      ...
      expect((message.body.c as Date).toUTCString()).toStrictEqual(cDate.toUTCString());
      expect(typeof message.body.d).toStrictEqual('object');
      expect(message.body._meta).toBeUndefined();
    });

    it('wrap should behave like Camunda Bpm Engine - meta object', () => {
      const [message] = SfnMessage.wrap(
      ...
      expect((message.body.c as Date).toUTCString()).toStrictEqual(cDate.toUTCString());
      expect(typeof message.body.d).toStrictEqual('object');
      expect(message.properties.customHeaders).toMatchSnapshot();
    });

    it('wrap should behave like Camunda Bpm Engine - lockExpirationTime', () => {
      const [message] = SfnMessage.wrap(
        Object.assign({}, awsMessage, {
          Body: JSON.stringify({
            body: { a: 1, b: true, c: cDate, d: { d1: new Date() }, e: [] },
      ...
      expect(message.properties.customHeaders.enteredTime.toISOString()).toStrictEqual(cDate.toISOString());
      expect(message.properties.lockExpirationTime).toBeDefined();
    });
